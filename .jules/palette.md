...
## 2025-10-24 - Qt Custom Context Menu Discoverability
**Learning:** In PySide6/Qt, components that rely on custom context menus (`Qt.ContextMenuPolicy.CustomContextMenu`) for core interactions (like editing or deleting items) lack visual discoverability by default. Users, especially those relying on screen readers or keyboard navigation, may not realize these actions are available.
**Action:** Always add explicit `setToolTip` and `setAccessibleDescription` to components with custom context menus to indicate the available actions (e.g., "Right-click to edit or delete"). This bridges the gap between hidden functionality and accessibility.
