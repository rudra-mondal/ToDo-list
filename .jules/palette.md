...
## 2025-10-23 - Ensuring WCAG AA Color Contrast in Qt Apps
**Learning:** Default text and border colors like `#888888` (light grey) and generic `red` can fail to meet WCAG AA contrast guidelines, which makes the UI inaccessible for visually impaired or colorblind users. This is especially problematic for completed task text, empty state labels, and inline validation errors.
**Action:** Replace low-contrast colors with accessible alternatives: Use darker greys like `#555555` instead of `#888888`, and high-contrast reds like `#CC0000` instead of default `red` for error borders and text.
