...
class TaskItemWidget(QFrame):
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton and not self.is_mini:
    ...
    def keyPressEvent(self, event):
        if not self.is_mini and event.key() == Qt.Key.Key_Menu:
            self.customContextMenuRequested.emit(QPoint(self.width() // 2, self.height() // 2))
            event.accept()
        elif not self.is_mini and event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            self.customContextMenuRequested.emit(QPoint(self.width() // 2, self.height() // 2))
            event.accept()
        else:
...
class MainWindow(QMainWindow):
    ...
    def _create_header(self):
        ...
        self.mini_btn.setStyleSheet(f"QPushButton{{border:2px solid transparent;background:{PRIMARY_COLOR};border-radius:16px;}} QPushButton:hover{{background:#D6655E;}} QPushButton:focus{{border:2px solid #555555;}}")
...
class MiniWindow(QMainWindow):
    ...
    def _create_header(self):
        ...
        back_btn.setStyleSheet(f"QPushButton{{border:2px solid transparent;background:{PRIMARY_COLOR};border-radius:14px;}} QPushButton:hover{{background:#D6655E;}} QPushButton:focus{{border:2px solid #555555;}}")
